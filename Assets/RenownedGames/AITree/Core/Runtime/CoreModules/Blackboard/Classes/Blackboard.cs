...
namespace RenownedGames.AITree
{
    ...
    public sealed class Blackboard : ScriptableObject, IBlackboard, IBlackboardKeys
    {
        ...
        public Key<T> SetFoundKey<T>(string name, Key<T> foundKey, T value)
        {
            System.Type keyType = typeof(Key<T>);
            if (TryFindKey(name, out Key key))
            {
                foundKey = key as Key<T>;
                foundKey.SetValue(value);
                return foundKey;
            }

            Debug.LogError("Key is not set");
